...
5. 先查看清理前盲點掃描與保守／平衡／積極方案預演，選擇方案後會切換到對應檢視並收合預演卡片；需要改方案時按「變更方案」。
...
- 聊天室與訊息瀏覽、搜尋、圖片預覽
- 附件分類、篩選、原圖／縮圖個別標記
- 清理前盲點掃描：SQLite 完整性、來源索引新鮮度、掃描狀態、WAL／SHM、未引用與不明附件
- 可選擇的保守／平衡／積極清理方案預演：保守方案可套用安全自動標記；平衡方案切換至 SQLite 未引用清單；積極方案切換至無法確認清單，兩者都必須人工複核
- 清理前檢查的非阻擋提醒以單行摘要呈現，詳細內容可按「查看提醒」展開，不會長時間佔用清理工作區
- 手動、自動與聊天室清理計畫分開記錄，可獨立清除手動標記
- 清理操作紀錄、可重現計畫 SHA-256 指紋與可複製的計畫摘要
- 建立候選檔後顯示 CRC、保留檔、SQLite 重寫、輸出數量與警告驗證報告
- 完全相同附件掃描與安全保留一份
- 大型備份的有界記憶體處理與可續跑工作
- FTS5 訊息搜尋與可觀察的首次索引進度；索引已建立後不會在每次搜尋重做完整來源內容驗證
